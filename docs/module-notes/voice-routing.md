...
- **web (fable):** `apps/local-web/src/composables/voice/**`, `components/voice/**`, `views/JarvisView.vue`. W1 subscribe with `wake=1|0` (capability) + parse the two new fields · W2 the drop guard becomes an own-session match (the command session exposes its live session id) · W3 the overlay-leg watchdog from `turnWatchdogMs` (default = the daemon default when the session had no wake).

Wire contract (the only cross-agent surface): subscribe query `surface` + `wake`; SSE `wake` = `{ kind, command, turnWatchdogMs }`; SSE `speak` = `{ kind, text, sessionId: string | null }`; daemon `POST /speak` body `{ text, sessionId?: string | null }`.

## Acceptance

- Window off + desktop app open (no Web Speech) → wake answered natively. Window on → unchanged.
- A schedule's `speak` during a Jarvis conversation plays in the Jarvis window once, nowhere else.
