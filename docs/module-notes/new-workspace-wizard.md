...
4. **Door + kickoff** — Chad's fork on every `openCreateWorkspace` entry point (Walk me through it · Pull from a folder → today's dialog · Create from a repository); Done → `addTab` + `ui.composerSeed`.
