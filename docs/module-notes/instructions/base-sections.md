...
| `workspace-manager.md` | — | **Kafi's Claude-web rewrite (2026-08-26, later):** a full persona — named by `{{workspace_name}}` (now RENDERED: the composer takes `workspaceName`, fail-loud on any unrendered `{{…}}`, wired through chat/channel/schedule/delegated doors), "you never build" (every change is a child's task in a worktree), first-turn ritual (duty book → journal → `state` memories → Features), one-area-one-persona, state kept as `state`-tagged memory entries, "Starting a new workspace" / "Managing an existing workspace" books (NOT WRITTEN YET), small-skips-ceremony-not-pipeline. Integration added Rule 7 naming the tools (`create_session`, `send_message` kind "task"). DECIDED (Kafi 2026-08-26): the merge session is the **Maintainer** — the workspace's standing developer persona: lands every merge into `main` (nothing else touches `main`), and takes small/immediate changes in a quick worktree it lands itself (Rule 4; small routes to it in Rule 7 + Each turn); `spawned-session.md` names it. Supersedes the older "the MANAGER merges". |
