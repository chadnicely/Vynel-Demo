...
| `workspace-manager.md` | — | **Kafi's Claude-web rewrite (2026-08-26, later):** a full persona — named by `{{workspace_name}}` (now RENDERED: the composer takes `workspaceName`, fail-loud on any unrendered `{{…}}`, wired through chat/channel/schedule/delegated doors), "you never build" (every change is a child's task in a worktree), first-turn ritual (duty book → journal → `state` memories → Features), one-area-one-persona, state kept as `state`-tagged memory entries, "Starting a new workspace" / "Managing an existing workspace" books (NOT WRITTEN YET), small-skips-ceremony-not-pipeline. Integration added Rule 7 naming the tools (`create_session`, `send_message` kind "task"). ⚠ OPEN: the rewrite lands worktrees via a **merge session** — contradicts the older locked "the MANAGER merges — never a child" and `spawned-session.md` (now reworded neutrally: "your manager's merge flow lands it"); Kafi to confirm the merge-session model, then the memory + duty books align. |
