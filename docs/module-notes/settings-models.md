...
| 5 ship | 2026-08-22 | the voice daemon rides the win-x64 payload as a second sidecar: `build-payload` bundles `apps/voice/src/main.ts` → `dist/voice.mjs` and seeds `@vynel/voice-daemon`'s third-party deps; `prune-payload` keeps only `node-cpal/bin/win32-x64` + `sherpa-onnx-win-x64`; `verify-payload` asserts the files and loads both natives with the staged runtime; the shell (`voice_sidecar.rs` over the shared `sidecar.rs` loop) allocates the voice port BEFORE the engine spawns (`choose_voice_port`, pinned by check-port-parity), passes `VYNEL_VOICE_DAEMON_URL` down, and starts the daemon once the engine answers. The daemon **boots idle** (`VoiceEngineSlot`): no model → no microphone leg, `/synthesize` answers 503, and the Voice screen's download → `POST /voice/reload` (`ready`) brings the voice live without a restart. |
