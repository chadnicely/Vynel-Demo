...
| 4 daemon wiring | `ed2bc293` | `VoiceEngines` holds the sherpa pair (always — wake + fallback) + stateless relay halves; getters route by source; `sessionRecognizer` vs the wake-pinned `recognizer`; driver `transcribeCommand` (state-based pick — asleep NEVER uses the cloud lane); `FallbackVoiceEngine` (thunked local fallback, never-silent); `planVoiceReload` passes sources through untouched (relays gate nothing on disk); reload outcome + api schema carry `ttsSource`/`sttSource`; daemon `wav-encode` died into the shared `pcm-codec` (one home). |
| 5 web | in worktree | Settings → Voice: Cloud voices cards (`VoiceProviderCard` — masked key, cleared on success, never echoed; live voices picker with language optgroups; Preview) + Hearing · conversation source radios (web-speech default); `use-voice-providers` composables; `cloud-command-recognizer` (AudioWorklet mic → `SpeechSegmenter` RMS endpointing + speech-RMS timer reset → WAV → `/api/voice/transcribe`) swapped in behind `CommandRecognizer` by `voiceSttSource`. Dictation (composer mic) stays web-speech-only in v1. |
