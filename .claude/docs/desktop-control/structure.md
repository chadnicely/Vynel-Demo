...
>
> **⚠ Partially stale (2026-08-11).** The desktop-control polish arc added plan-level approval (`src/plan/`), batched actions, installed-app listing + launch (`src/apps/`), and window state (`a11y/window-state.ts`, `a11y/window-selection.ts`, `a11y/powershell.ts`). The approval-tier statements below are corrected; the file map and tool table are NOT yet regenerated. Current record: `docs/module-notes/desktop-control-plan-approval.md`.
>
...
| `packages/desktop-control/src/a11y/screenshot-adapter.ts` | the SINGLE node-screenshots require point (lazy, exported as `loadNodeScreenshots`): `screenshotApp` (BitBlt-class capture WITHOUT focusing; zoom via `cropSync` at full res; full window downscaled), `findAppWindowBounds` (the act coordinate frame, carries `appName` for enforcement), pure `selectWindowId`/`clampZoomRegion` |
| `packages/desktop-control/src/a11y/window-identity.ts` | identity for ENFORCEMENT, over the shared loader: `findFocusedWindowAppName` (focus-directed target), `findWindowAppNameAtPoint` + pure z-sorted `pickTopmostWindowAt` (hit-test for absolute coords), `findAppNameByPid` / `resolveAppIdentity` (**the canonical grant identity** — maps a pid to the real app name so a window TITLE is never used as a grant key), `listWindowAppNames` (grant-door source) |
...
| ► `packages/desktop-control/src/mcp/desktop-mcp-feature-descriptor.ts` | `desktopFeatureDescriptor` — `build` returns `null` when `context.desktopReader === undefined`; `mutatingToolNames: ['mcp__desktop__request_desktop_access']` (cards in ask + unattended; uncarded in user auto/bypass); `askModeApprovalToolNames: ['mcp__desktop__propose_desktop_plan']` (the act tools LEFT this tier — the plan envelope gates them in-tool); `contributePrompt` |
...
- **Two approval tiers, disjoint by design.** `mutatingToolNames: ['mcp__desktop__request_desktop_access']` — the STANDING-grant consent tool cards in ask + the unattended background default (a grant born without a card would hollow out the model), uncarded in the user's own auto/bypass. `askModeApprovalToolNames: ['mcp__desktop__propose_desktop_plan']` — **the plan is the one card of a desktop task** (Kafi 2026-08-11): the act tools LEFT this tier and are gated in-tool by the turn's plan envelope instead, so an approved plan runs its steps card-free. Both declared unconditionally; the tiers are additive so declaring an unregistered tool is harmless.
