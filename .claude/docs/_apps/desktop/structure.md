...
>
> Folders touched: `apps/desktop/src-tauri/src/` · `apps/desktop/src-tauri/{capabilities,tauri.conf.json,Cargo.toml}` · reads from `apps/local-api/src/{server,gateway}.ts` · loads `apps/local-web` views · launched by `apps/voice/src/overlay/display-dock-window.ts`
...
| Path | Role |
|---|---|
| ► `src-tauri/src/main.rs` | Tauri entry — `main()` builds the app, reads the `--dock-only` arg, in `setup()` branches dev→`windows::create_windows` / release→`daemon::ensure_daemon_then_open_windows`; `on_window_event` quits when the `main` window closes; `RunEvent::Exit`→`daemon::stop()` |
...
| `src-tauri/src/windows.rs` | window creation in code (so release can delay it until the daemon binds) — the `main` app window (unless `--dock-only`) + the always-on-top transparent `display-dock` overlay |
...
| `src-tauri/capabilities/display-dock.json` | Tauri v2 capability grant — the `display-dock` window's self-management permissions (show/hide/focus/set-position/start-dragging) |
...
`create_windows(handle, dock_only)` builds up to two `WebviewWindow`s:

| Window | Label | URL (`WebviewUrl::App`) | Flags |
|---|---|---|---|
| Main app | `main` | `/` | 1280×800, min 960×600, standard chrome. **Skipped when `--dock-only`** |
| Display dock | `display-dock` | `/display-dock` | 420×560, `decorations(false)`, `transparent(true)`, `shadow(false)`, `always_on_top(true)`, `resizable(false)`, `skip_taskbar(false)` — always built |

The overlay manages its own show/hide/park/drag from the web side (`apps/local-web/src/composables/voice/tauri-overlay-window.ts`), which is exactly what the `display-dock-window` capability grants (`capabilities/display-dock.json`).

### The `--dock-only` wake path

The voice daemon relaunches this same exe with `--dock-only` on a wake word when no window is connected (`apps/voice/src/overlay/display-dock-window.ts:75`). `main.rs:16` reads the flag; `create_windows` then builds **only** the overlay, leaving the full app closed. The daemon sidecar is still ensured in release because the overlay's UI is served by it.
...
- **Single-instance ownership hole** (`main.rs:34–42`) — the close-quits-app promise only holds within ONE process. A `--dock-only` process has no `main` window (no exit path) and a second full instance that attaches to the first's daemon loses it when the first closes. D2: single-instance plugin + explicit daemon ownership.
...
| [local-web](../local-web/overview.md) | out | webview URL load | `main`→`/`, `display-dock`→`/display-dock`; overlay control via `composables/voice/tauri-overlay-window.ts` |
| [voice](../../voice/overview.md) | in | process launch | `display-dock-window.ts` runs this exe `--dock-only` on wake |
| Tauri v2 runtime | out | crate dep | `tauri = "2"` — `Builder`, `WebviewWindowBuilder`, `AppHandle`, capability system |

```mermaid
flowchart LR
    voice[voice daemon] -- "--dock-only" --> D[desktop shell]
    D -- "spawn node" --> api[local-api :18892]
    D -- "load / and /display-dock" --> web[local-web views]
    web -. "served by" .-> api
