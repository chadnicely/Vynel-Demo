...
>
> Folders touched: `apps/voice/src/` · `apps/local-api/src/routes/voice/` · `apps/local-api/src/gateway.ts` · `apps/local-web/vite.config.ts` (`/voice` proxy) · `apps/desktop/src-tauri/src/` (the display-dock window)
...
| `@vynel/voice-daemon` (`apps/voice`, **this unit**) | **the shell** — imperative I/O | mic/speaker via node-cpal, the loop state machine, the brain SSE client, the overlay HTTP channel, the display-dock launcher, env | stateful, device-bound |
...
| Path | Role |
|---|---|
| ► `apps/voice/src/main.ts` | the composition root — loads env + models, builds the audio shell, overlay channel, display-dock launcher and `VoiceSessionDriver`, wires the late-bound `io`/`wakeHandoff` seams, registers SIGINT/SIGTERM shutdown |
...
| `apps/voice/src/overlay/overlay-channel.ts` | the loopback Hono server (port 18893) for browser voice views — SSE `/events`, `/session/end`, `/speak`, `/synthesize` |
| `apps/voice/src/overlay/display-dock-window.ts` | launch/focus the display dock — Tauri overlay exe preferred, Chrome/Edge `--app` fallback; an exe that exits within 3 s of launch (crash, stale build) triggers the browser fallback so the wake is still answered |
| `apps/voice/src/**/*.test.ts` | colocated Vitest tests: `audio-format`, `wav-encode`, `run-brain-turn`, `sse-frames`, `voice-session-driver`, `display-dock-window`, `overlay-channel` |
...
5. `wakeSurface` is `'dock'` when `VYNEL_VOICE_DOCK_WINDOW === '1'` (default) — only the floating window runs wakes; otherwise `'any'`.
6. Build `createDisplayDockWindow` (browser/url/appPath config) and the `VoiceSessionIo` that mirrors every state change to **both** the audio shell (status log) and the overlay (SSE publish).
...
If the overlay channel fails to bind (port taken), `overlay.whenListening.catch` stops everything and hard-exits — without the channel the display dock can never connect (`main.ts:87-97`).
...
| Method | Path | Purpose |
|---|---|---|
| GET | `/events?surface=app\|dock` | SSE — replays last state on connect, then `{kind:'state'}` / `{kind:'wake'}` frames. Wake goes to **one** newest eligible client (two would answer twice) |
...
An undelivered wake is held (`pendingWake`) and replayed to the next eligible connect — that is how the same-breath command survives the display dock's launch time, and how a wake lost to a dying socket recovers (`overlay-channel.ts:27-98`).
...
flowchart TD
    ...
    WW -- dock mode --> HO[handed-off: publishWake + open the display dock]
...
3. Asleep: `detectWakeWord` (`@vynel/voice`). Dock mode → `#state = 'handed-off'`, `publishWake` + open/focus the window (`voice-session-driver.ts:231-245`, `main.ts:132-151`). Native → run the turn.
...
| local-web `/display-dock` | both | SSE + launched window | wake/state events out; the browser owns the command session |
| [desktop](../desktop/overview.md) (Tauri) | out | `spawn` | on wake, launches `vynel-desktop.exe --dock-only` for the overlay |
...
flowchart LR
    ...
    web[local-web /display-dock] -. SSE .- VD
    VD -. spawn --dock-only .-> desk[desktop Tauri]
```
...
- **Nothing auto-supervises the voice daemon yet.** The desktop's `daemon.rs` supervises the **local-api brain** (port 18892), not this daemon. The relationship runs the other way: on a wake the voice daemon *launches* the desktop overlay window (`display-dock-window.ts` → `vynel-desktop.exe --dock-only`, `apps/desktop/src-tauri/src/main.rs:16`), or a Chrome/Edge `--app` window on local-web's `/display-dock` route as fallback.

## Config & gotchas

- **Env** (`env.ts`): `VYNEL_API_URL` (default `http://127.0.0.1:18892`), `VYNEL_VOICE_MODELS_DIR` (`.models/voice`), `VYNEL_VOICE_TTS` (`kokoro`|`piper-lessac`), `VYNEL_VOICE_STT` (`moonshine-base`|`moonshine-tiny`), `VYNEL_VOICE_ID`, `VYNEL_VOICE_IDLE_TIMEOUT_MS` (15 s), `VYNEL_VOICE_DAEMON_PORT` (18893), `VYNEL_VOICE_DOCK_WINDOW` (`1`), `VYNEL_VOICE_DOCK_URL` (`…:18894/display-dock`), `VYNEL_VOICE_DOCK_BROWSER`, `VYNEL_VOICE_DOCK_APP` (Tauri exe path).
...
- **Dock connect watchdog** — if a launched window never connects within 10 s (`DOCK_CONNECT_TIMEOUT_MS`), the daemon abandons the handoff and resumes wake-listening so a failed launch never leaves it deaf (`main.ts:29,142-149`).
