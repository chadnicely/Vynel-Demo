...
- **`status` / `kind` / `tier` / `recommendedScope` are plain `text` columns.** `publish-input.ts` validates them as enums at write; reads coerce legacy/unknown values back to the wire enum in one home (`normalizeTier`, `normalizeScope` in `admin-catalog.ts`, the `=== 'community' ? … : 'verified'` guards in the mappers).
