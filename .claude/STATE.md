...
## Repos & branch
- **Working:** `E:\KLONE\Workspace\vynel` — git `main`, remote `github.com/kafijunior/vynel-beta`, pushed through `36088b8`.
...
6. **Response schemas (B)** *(green + staged; commit pending Chad)* — the 5 knowledge routes declare response schemas (`resolver()` on each 200); `Serialized*` types derive from them via `z.infer` (one source, −50 lines). SDK returns are now **typed** (`client.knowledge.search()` → `{ results: […] }`), flat + namespaced. `expectTypeOf` guard per route.
**Gate:** `pnpm install` exit 0 · `turbo typecheck` all green · `pnpm test:parity` (schema 29 · mcp · sdk) · `vitest` 497 passed / 4 skipped. **Full `pnpm test` green.**

## NEXT: Step D — the CLI (`apps/cli`)
Net-new `apps/cli` (`@vynel/cli`) over the namespaced SDK — direction ①. No KAFI/letterman reference.
`vynel knowledge <search|list|get|status|reindex> --workspace <id>` via `commander`;
`createVynelClient({ baseUrl })` (baseUrl from `env.ts`); thin testable handlers (stub-client tests);
`SdkError` → stderr + non-zero exit. Then:
