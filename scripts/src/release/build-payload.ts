...
function installThirdPartyDependencies(
  backendDir: string,
  target: PayloadTarget,
): void {
...
const PREBUILD_INSTALL_DEPENDENCIES = [
  {
    ...
    binaryPath: join("build", "Release", "better_sqlite3.node"),
  },
];

function repairCrossBuiltNatives(
  backendDir: string,
  target: PayloadTarget,
): void {
  ...
  for (const { packageName, binaryPath } of PREBUILD_INSTALL_DEPENDENCIES) {
    ...
    if (
      existsSync(shippedBinary) &&
      readNativeBinaryFormat(shippedBinary) === expected
    )
      continue;
    console.log(
      `build-payload: fetching ${target.id} prebuild for ${packageName}`,
    );
    run(
      "node",
      [
        join(nodeModulesDir, "prebuild-install", "bin.js"),
        `--platform=${target.os}`,
        `--arch=${target.cpu}`,
      ],
      packageDir,
 ...
 */
function sweepForeignNativeBinaries(
  ...
  const walk = (directory: string): void => {
    for (const entry of readdirSync(directory, { withFileTypes: true })) {
      ...
      if (
        OPTIONAL_NATIVE_MARKERS.some((marker) => entryPath.includes(marker))
      ) {
        console.log(
          `build-payload: removing host-built optional native ${entryPath}`,
        );
...
function copyAssets(backendDir: string): void {
  ...
  cpSync(
    join(repoRoot, "packages", "db", "src", "migrations-sqlite"),
    join(assetsDir, "migrations-sqlite"),
    {
      recursive: true,
    },
  );
  ...
  for (const contentDir of [
    ...
    cpSync(
      join(instructionsRoot, contentDir),
      join(assetsDir, "instructions", contentDir),
      {
        recursive: true,
      },
    );
...
function copyWebUi(payloadDir: string, skipWebBuild: boolean): void {
  if (!skipWebBuild)
    run("pnpm", ["--filter", "@vynel/local-web", "build"], repoRoot);
  const webDist = join(repoRoot, "apps", "local-web", "dist");
  if (!existsSync(join(webDist, "index.html"))) {
    throw new Error(
      `No built web UI at ${webDist} — run without --skip-web or build local-web first.`,
    );
  }
...
async function main(): Promise<void> {
  ...
  const target = resolvePayloadTarget(
  ...
  const sourcemapDir = join(
    payloadDir,
    "..",
    target.os === "win32" ? "payload-sourcemaps" : `${target.id}-sourcemaps`,
  );
  ...
  console.log(
    `build-payload: pruned dead weight — ${formatPruneReport(pruneReport)}`,
  );
  ...
  await stageToolPolicyMap(backendDir);
  copyWebUi(payloadDir, skipWebBuild);
  const stagedNode = await stageNodeRuntime({
    target,
  ...
  console.log(
    "build-payload: next, run verify-payload.ts against this payload.",
  );
}
