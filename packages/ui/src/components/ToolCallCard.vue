...
// A BLOCKED call (the provider's own safety check refused it before it ran)
...
// The blocked line keys off the STATUS (the row's truth); the reason comes
// from the refusal record the row carries, when the provider gave one.
...
<template>
  <div class="tool-call-card" :class="{ 'is-expanded': isExpanded }">
    ...
    <!-- The refusal line: who stopped it, why, and the one way forward. -->
    <div v-if="isBlocked" class="blocked-line" data-testid="tool-call-blocked">
...
/* The refusal line reads like a Claude Code denial note: a quiet row under
   the chip, the danger hairline on the left, the action flush right. */
.blocked-line {
  display: flex;
  align-items: center;
  gap: 8px;
