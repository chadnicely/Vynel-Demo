...
describe('findSessionStatusMessageFacts', () => {
  ...
  // Kafi's one-rule decision (2026-08-17): ANY error the user can see is a
  // problem. "The engine isn't reachable" is the most actionable failure the
  // app has, and it is the ONE the provider marks recoverable — so exempting
  // recoverable failures (briefly shipped, then reverted) hid exactly the
  // error that most needs saying.
  it('an unreachable engine IS a problem, recoverable or not', async () => {
    await withTestDatabase((db) => {
      const user = makeUser()
      insertUser(db, user)
      const workspace = makeWorkspace(user.id)
      ...
      insertChatMessage(
        ...
        makeChatMessage(session.id, {
          ...
          errorMessage: 'The Claude engine did not respond. Check that it is running and signed in.',
          startedAt: new Date('2026-08-16T12:56:00Z'),
        }),
      )
      expect(findSessionStatusMessageFacts(db, [session.id]).lastAssistantError?.code).toBe(
