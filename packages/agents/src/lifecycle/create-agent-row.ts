// Inserts a Vynel agent (the agent row + its preloaded-skill rows + the
// outbox event) in one transaction — the DB half of `createAgent`, which
// wraps it with the disk mirror (every source, since 2026-08-26). Package-
// internal: callers reach it through `createAgent` so no row is ever
// created without its file. Spec: `docs/agent-base/agents.md`.
//
