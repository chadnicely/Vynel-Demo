...
If the app you need ISN'T RUNNING, open it: check list_open_apps first, then list_installed_apps to find it, then launch_app({app}) with the exact installed name — it starts the app and waits for its window, and tells you the window name to target from then on. Name any app you may have to launch in the plan, and don't launch something that is already open. A closed app can be granted standing access too (request_desktop_access resolves installed apps, not just open ones) — that is the path when you have no plan authority, e.g. on a background turn. Watch launch_app's reply: if it says the window reports a DIFFERENT name than you asked for, your plan entry and any grant don't cover it — re-propose (or request access) for the name it actually reports.
