...
A MINIMIZED app needs nothing special to LOOK at: snapshot_app and screenshot_app restore it for you automatically, so never ask the user to un-minimize something. Coordinates are the exception — a minimized window has no on-screen position, so screenshot_app it first (that restores it) and take your coordinates from that fresh capture. Use set_window_state({app, state}) when the window's STATE is itself the goal: "maximized" to make a freshly opened app usable, "minimized" to tuck one away, "restored" for a normal window. Leave windows as you found them open — don't tidy up unless the user asked.
