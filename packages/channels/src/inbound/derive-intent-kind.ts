...
export function deriveIntentKind(messageBody: string): InboundIntentKind {
  const trimmed = messageBody.trim()
  // Slash commands (/help, /status, /new) — Phase 1.5 handlers; classified now.
  if (trimmed.startsWith('/')) return 'channel-command'
