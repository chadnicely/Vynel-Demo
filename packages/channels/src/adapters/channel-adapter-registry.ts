...
// lazy singleton per kind. Phase 1 ships only `TelegramChannelAdapter`;
// `'discord'` throws a clear `ValidationError` (Phase 1.5 — D15). Mirrors
// the providers `resolveAiAgentProvider` precedent (one instance per kind
// for the life of the process), but instantiates lazily on first use.
//
