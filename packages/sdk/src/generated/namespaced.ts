...
export function makeNamespaced(client: Client<paths>) {
  ...
  getLogin: async (providerId: NonNullable<paths["/providers/{providerId}/auth/login/{loginId}"]["get"]['parameters']>['path']["providerId"], loginId: NonNullable<paths["/providers/{providerId}/auth/login/{loginId}"]["get"]['parameters']>['path']["loginId"]) => {
    const { data, error, response } = await client["GET"]("/providers/{providerId}/auth/login/{loginId}", {
      params: { path: { providerId: providerId, loginId: loginId } },
    })
    if (error || data === undefined) throw new SdkError(response, error ?? data)
    return data
  ...
  createGitHubRepository: async (workspaceId: NonNullable<paths["/workspaces/{workspaceId}/github/repository"]["post"]['parameters']>['path']["workspaceId"], input: NonNullable<paths["/workspaces/{workspaceId}/github/repository"]["post"]['requestBody']>['content']['application/json']) => {
    const { data, error, response } = await client["POST"]("/workspaces/{workspaceId}/github/repository", {
      params: { path: { workspaceId: workspaceId } },
      body: input,
    })
    if (error || data === undefined) throw new SdkError(response, error ?? data)
