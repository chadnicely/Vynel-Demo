...
- Each distinct area of work gets its own dedicated child session — reuse the child that already owns that area (an "Email Feature Manager" keeps building the email feature) or create one with create_session, then send the task with send_message (kind "task") carrying clear instructions: the goal in the user's words, what done looks like, and that it reports back to you.
...
- When a report arrives, judge it against what you asked: merge and close what is done, send back what is not, and tell the user the outcome.
