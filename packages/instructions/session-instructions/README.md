...
| File | Governs |
|---|---|
| `base.md` | The TEXT base — every text session's shared identity, laid out in the same section order as Claude Code's preset so the two compare line by line (`docs/module-notes/instructions/base-sections.md` is the status map): `# Identity` (Claude, working through the Claude Agent SDK inside Vynel; the role is the kind file attached after it, which wins where the two differ; `whoami` when unsure; newest instructions win over old habits), the security paragraph (Anthropic's, verbatim), `# Harness` (ours: how text reaches the user, the approval card + declined calls, `<system-reminder>` tags, tool hygiene, "check the environment with your tools"), `# Continuity` (the four places: Memory — what holds · Journal — what happened · Knowledge base — what you'd look up · Notebook — how we work, incl. the Coding Guideline book), `# Context management` (a session is a persona, not a context window; the alert path ends in `checkpoint`), `# Session-specific guidance` (verbatim), `# Working with the user`, `# Pronouns` (verbatim), `# Output format` (the UI-load-bearing step-line rules + reply shape). |
...
| `workspace-manager.md` | KIND: the Workspace Manager persona — the workspace primary session, on every one of its turns (interactive chat, channel turns, schedule fires, and routed background turns alike). Templated — `{{workspace_name}}` is filled by `composeSessionInstruction('workspace-manager', { workspaceName })` (keep the placeholder when editing; the composer fails loudly on any unrendered `{{…}}`). It never builds: every change is a task for a child persona (one area = one persona) in its own worktree, landed by the **Maintainer** — the standing developer persona that alone touches `main` and also takes small, immediate changes. |
