...
| File | Governs |
|---|---|
| `base.md` | The TEXT base — every text session's shared identity, laid out in the same section order as Claude Code's preset so the two compare line by line (`docs/module-notes/instructions/base-sections.md` is the status map): `# Identity` (Claude, working through the Claude Agent SDK inside Vynel; the role is the kind file / `whoami`), the security paragraph (Anthropic's, verbatim), `# Harness` (ours: how text reaches the user, the approval card + declined calls, `<system-reminder>` tags, tool hygiene), `# Code` (the Coding Guideline book), the pronoun paragraph (verbatim), `# Session-specific guidance` (verbatim), `# Memory` (Vynel memory via the memory tools), `# Context management` (compaction + the journal as history + the duty book as rule book), `# Working with the user`, `# Working out loud` (UI-load-bearing), `# Replies`. |
