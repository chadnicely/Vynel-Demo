...
function seedSession(db: Database, userId: string, workspaceId: string, model: string | null) {
  const now = new Date()
...
describe('attachDeliveredRunStats', () => {
  it('a delivered row gains the producing run stats: model, tool calls, token sums, claim→report duration', async () => {
    await withTestDatabase((db) => {
      ...
      const rowId = seedTracedAssistantRow(db, sessionId, work.partialSessionId!, {
        input: 100,
        output: 50,
      })
      seedTracedAssistantRow(db, sessionId, work.partialSessionId!, { input: 200, output: 25 })
      ...
      completeDelegationJob(db, jobId, 'done', new Date('2026-08-09T10:00:06Z'))
      ...
      const deliveryKey = findDelegationJobById(db, deliveryId)!.partialSessionId!
  ...
  it('falls back to the SESSION model without an override, and passes through when the job is gone', async () => {
    await withTestDatabase((db) => {
      ...
      const work = findDelegationJobById(db, jobId)!
      const sessionId = seedSession(db, user.id, workspace.id, 'claude-sonnet-y')
      seedTracedAssistantRow(db, sessionId, work.partialSessionId!, { input: 10, output: 5 })
      ...
      const [withStats, orphan] = attachDeliveredRunStats(db, [
        { id: 'd1', role: 'user', sourceKind: 'agent', partialSessionId: deliveryKey },
        { id: 'd2', role: 'user', sourceKind: 'agent', partialSessionId: randomUUID() },
      ])
      expect((withStats as { runStats?: { model: string | null } }).runStats?.model).toBe(
        'claude-sonnet-y',
      )
