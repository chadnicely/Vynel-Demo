// A configurable fake `AiAgentProvider` for the primary-as-thread swap-wiring
// tests — lets `runSeededSwapSession` + `bridgePrimarySessionAfterTurn` be
// exercised end-to-end WITHOUT a live SDK. Only the two methods the swap path
// uses (`startChatSession` for the seeded priming turn, `summarizeSession` for
// the carry) are configurable; every other abstract method throws if reached,
// so a test that accidentally hits one fails loudly. Test-support only — never
