...
// model (the voice daemon's latency tier): resuming a session whose context
// occupancy already exceeds the pin's window guarantees the provider dies with
// "Prompt is too long" before doing anything — on a hands-free surface, with
// nobody watching an error row. Live incident 2026-08-19: the global brain sat
// at ~443k tokens (grown under 1M-window models, legitimately below the 0.85
// swap threshold), and every haiku-pinned voice turn hard-failed.
//
// The decision is deliberately conservative and READ-only:
//   pin fits          -> run the pin (the surface's latency choice stands).
//   pin can't hold it -> run the segment's own last-ran model — the context
//                        provably fits it; that is the model that grew it.
...
// the swap threshold is not a fit.
//
...
import { detectContextPressure } from '../continuity/index.js'
...
export type FitPinnedModelToSessionResult = {
  /** The model the turn should run — the pin when it fits, else the segment's
   *  last-ran model, else undefined (the engine default). */
...
export function fitPinnedModelToSession(
  ...
  const sessionModel = segment?.model ?? null
  return {
    model: sessionModel !== null && fits(sessionModel) ? sessionModel : undefined,
    wasReplaced: true,
    occupancyTokens,
  }
}
