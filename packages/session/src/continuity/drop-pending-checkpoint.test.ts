...
describe('dropPendingCheckpoint', () => {
  ...
  it('names every reason in plain words', () => {
    ...
    expect(composeDroppedCheckpointNote('x', 'never-continues')).toContain('(this kind of turn never continues work automatically)')
    expect(composeDroppedCheckpointNote('x', 'left-behind')).toContain('(an earlier turn ended without continuing it)')
  })
})
