// Integration tests for the USER-scoped approvals HTTP surface (`/approvals`,
// `./user-scoped.ts`) — real SQLite via withTestDatabase, the provider mocked
// at the module boundary (resolveApproval calls respondToApprovalRequest to
// unblock the paused agent). The user is the Phase-1 single local user that
// userResolverMiddleware resolves, so seeds use the same getOrCreateLocalUser
// identity the route will see.
