...
//   POST   /voice/latency-trace   -> one spoken exchange's stopwatch, into the server log (NOT a tool)
...
import {
  ...
  VoiceLatencyTraceRequestSchema,
  VoiceLatencyTraceResponseSchema,
...
export const voiceApp = factory
  ...
  .post(
    ...
    describeRoute({
      ...
      responses: {
        200: {
          description:
            "{ spoken: true } — or { spoken: false, reason } if voice output isn't available.",
          content: {
            "application/json": { schema: resolver(SpeakResponseSchema) },
          },
        },
      },
      "x-mcp": {
    ...
    async (c) => {
      ...
      if (c.var.remoteEngine) {
        return c.json({
          spoken: false,
          reason:
            "voice output lives on the desktop; this engine runs on a remote server",
        });
      }
      if (callId !== undefined) {
        const spoken = await speakIntoCallThroughDaemon(
          loadEnv().VYNEL_VOICE_DAEMON_URL,
          callId,
          text,
        );
        return c.json(
          spoken.ok
            ? { spoken: true }
            : { spoken: false, reason: spoken.reason },
        );
      }
      ...
      const sessionId =
        parseTurnSessionHeader(c.req.header(TURN_SESSION_HEADER)) ?? null;
      return c.json(
        await speakThroughDaemon(
          loadEnv().VYNEL_VOICE_DAEMON_URL,
          text,
          sessionId,
        ),
  ...
  .post(
    ...
    describeRoute({
      ...
      responses: {
        200: {
          description:
            "{ stopped: true } — or { stopped: false, reason } when nowhere could hear the stop.",
          content: {
            "application/json": {
              schema: resolver(StopListeningResponseSchema),
            },
          },
        },
      },
    ...
    async (c) => {
      ...
      if (sink === undefined && !daemonStopped) {
        return c.json({
          stopped: false,
          reason: "no voice window or daemon could hear the stop",
        });
      }
    ...
    describeRoute({
      ...
      responses: {
        200: {
          ...
          content: {
            "application/json": { schema: resolver(VoiceReloadResponseSchema) },
          },
        },
      },
    }),
    ...
    async (c) => {
      if (c.var.remoteEngine) {
        return c.json({
          reloaded: false,
          reason:
            "voice lives on the desktop; this engine runs on a remote server",
        });
      }
      return c.json(
        await reloadVoiceThroughDaemon(loadEnv().VYNEL_VOICE_DAEMON_URL),
      );
    },
  )
  .post(
    ...
    describeRoute({
      ...
      responses: {
        200: {
          description:
            "{ published } — false when this engine has no live channel to fan it over.",
          content: {
            "application/json": {
              schema: resolver(DisplayActiveResponseSchema),
            },
          },
        },
      },
    ...
    describeRoute({
      ...
      responses: {
        200: {
          ...
          content: {
            "application/json": {
              schema: resolver(DisplaySessionResponseSchema),
            },
          },
        },
      },
  ...
  .post(
    ...
    describeRoute({
      ...
      "x-sdk-name": "voice.reportLatencyTrace",
      responses: {
        200: {
          description: "{ logged: true } — the trace landed in the engine log.",
          content: {
            "application/json": {
              schema: resolver(VoiceLatencyTraceResponseSchema),
            },
          },
        },
      },
      // No x-mcp on purpose: a stopwatch reading is the WINDOW talking to the
      // engine's log, not a capability the model may reach for.
    }),
    validator("json", VoiceLatencyTraceRequestSchema),
    ...userScoped,
    (c) => {
      const trace = c.req.valid("json");
      // INFO, one line per exchange: this is the deliverable — the numbers in
      // the dev terminal. The message carries the one figure Chad feels; the
      // structured payload carries every mark for the tuning pass.
      c.var.logger.info(
        { voiceLatency: trace },
        trace.complete
          ? `voice latency: ${trace.speechEndToFirstAudibleMs ?? "?"}ms speech->first-sound (dispatch ${trace.speechEndToDispatchMs ?? "?"} + model ${trace.dispatchToFirstTokenMs ?? "?"} + tts ${trace.firstTokenToFirstTtsMs ?? "?"} + play ${trace.firstTtsToFirstAudibleMs ?? "?"})`
          : `voice latency: partial trace (${trace.reason ?? "no reason"})`,
      );
  ...
  .post(
    ...
    describeRoute({
      ...
      responses: {
        200: {
          ...
          content: {
            "application/json": { schema: resolver(StartCallResponseSchema) },
          },
        },
      },
      "x-mcp": {
    ...
    ...userScoped,
    async (c) => {
      const { label, mode, goal, capturePid, captureProcessName } =
        c.req.valid("json");
      if (c.var.remoteEngine) {
        return c.json({
          started: false,
          reason:
            "calls run on the desktop; this engine runs on a remote server",
        });
      }
      ...
      const created = await createSpawnedSession(c.var.db, c.var.aiProvider, {
        ...
        name: label,
        purpose: buildCallSessionPurpose({
          label,
          mode,
          assistantName: CALL_ASSISTANT_NAME,
          goal,
        }),
  ...
  .get(
    ...
    describeRoute({
      ...
      responses: {
        200: {
          description:
            "{ calls } — empty when not in any call (or on a remote engine).",
          content: {
            "application/json": { schema: resolver(ListCallsResponseSchema) },
          },
        },
      },
      "x-mcp": {
    ...
    async (c) => {
      ...
      const listed = await listCallsThroughDaemon(
        loadEnv().VYNEL_VOICE_DAEMON_URL,
      );
  ...
  .delete(
    ...
    describeRoute({
      ...
      responses: {
        200: {
          description:
            "{ ended: true, sessionId } — or { ended: false, reason } when the call was already gone.",
          content: {
            "application/json": { schema: resolver(EndCallResponseSchema) },
          },
        },
      },
      "x-mcp": {
    ...
    async (c) => {
      ...
      const ended = await endCallThroughDaemon(
        loadEnv().VYNEL_VOICE_DAEMON_URL,
        callId,
      );
      ...
      return c.json({
        ended: true,
        ...(ended.value.sessionId !== undefined
